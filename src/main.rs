...
// size and coordinates relates to GUI
...
fn place_objects(room: Rect, map: &Map, objects: &mut Vec<Object>) {
    ...
    for _ in 0..num_monsters {
        ...
        // first check whether the tile isn't occuppied
        if !is_blocked(x, y, map, objects) {
            // it's free, let's create monster
    ...
    for _ in 0..num_items {
        ...
        if !is_blocked(x, y, map, objects) {
            ...
            let mut item = if dice < 0.7 {
                // create a healing option (70% chance)
...
fn player_move_or_attack(dx: i32, dy: i32, game: &mut Game, objects: &mut [Object]) {
    // aim tile coordinates
    let new_x = objects[PLAYER].x + dx;
    let new_y = objects[PLAYER].y + dy;

    // try to find attackable object there
    let target_id = objects
        .iter()
        .position(|object| object.fighter.is_some() && object.pos() == (new_x, new_y));
...
fn pick_item_up(object_id: usize, game: &mut Game, objects: &mut Vec<Object>) {
    if game.inventory.len() >= INVENTORY_SIZE {
        ...
        game.messages
            .add(format!("You picked a {}", item.name), GREEN);
        game.inventory.push(item);
    }
}

/// find closest enemy, up to a maximum range, ano in the player's FOV
...
fn cast_heal(
    ...
    if let Some(fighter) = objects[PLAYER].fighter {
        ...
        game.messages.add("You feel mutch better!", LIGHT_VIOLET);
...
fn cast_lightning(
    ...
    if let Some(monster_id) = monster_id {
        ...
        // no monster found within maximum range
...
fn cast_confuse(
    ...
    let monster_id = target_moster(tcod, game, objects, Some(CONFUSE_RANGE as f32));
    if let Some(monster_id) = monster_id {
...
fn level_up(tcod: &mut Tcod, game: &mut Game, objects: &mut [Object]) {
    ...
    if player.fighter.as_ref().map_or(0, |f| f.xp) >= level_up_xp {
        ...
        while choice.is_none() {
            // keep asking until choice is made
            choice = menu(
...
fn player_death(player: &mut Object, game: &mut Game) {
    ...
    // transform the player into corpse!
    player.char = '%';
...
fn monster_death(monster: &mut Object, game: &mut Game) {
    ...
    game.messages.add(
        format!(
            "{} is dead! Your gain {} experience points.",
            monster.name,
...
fn make_map(objects: &mut Vec<Object>) -> Map {
    ...
    // NOTE: works only when the player is the first element!
    ...
    for _ in 0..MAX_ROOMS {
        ...
        // if there are no intersections, so this room is valid
        if !failed {
...
fn handle_keys(tcod: &mut Tcod, game: &mut Game, objects: &mut Vec<Object>) -> PlayerAction {
    ...
    match (tcod.key, tcod.key.text(), player_alive) {
        ...
        (Key { code: Text, .. }, "g", true) | (Key { code: NumPad5, .. }, _, true) => {
            // pick up a item
            let item_id = objects
                .iter()
        ...
        (Key { code: Text, .. }, "c", true) => {
            ...
            if let Some(figther) = player.fighter.as_ref() {
                let msg = format!(
...
Attack: {}
Defense: {}",
                    level, figther.xp, level_up_xp, figther.max_hp, figther.power, figther.defense
                );
...
fn get_names_under_mouse(mouse: Mouse, objects: &[Object], fov_map: &FovMap) -> String {
    ...
    // create list with the names of all objects at mouse's coordinates and in FOV
    let names = objects
        .iter()
    ...
    names.join(", ") // retun names, separated by commas
}
...
fn menu<T: AsRef<str>>(header: &str, options: &[T], width: i32, root: &mut Root) -> Option<usize> {
    ...
    for (index, option_text) in options.iter().enumerate() {
        let menu_letter = ('a' as u8 + index as u8) as char;
...
fn render_all(tcod: &mut Tcod, game: &mut Game, objects: &[Object], fov_recompute: bool) {
    ...
    // pick into mutable vector only visible objects
...
fn new_game(tcod: &mut Tcod) -> (Game, Vec<Object>) {
    ...
    // welcoming message
    game.messages.add(
        "Welcome stranger! Prepare to explore Tombs of Dead King!",
        RED,
    );
